...
$old_error_handler = set_error_handler("control_atestado_aluno_scriptcase_error_handler");

function control_atestado_aluno_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['control_atestado_aluno']['contr_erro']) && $_SESSION['scriptcase']['control_atestado_aluno']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_control_atestado_aluno->contr_control_atestado_aluno->Apl_com_erro = true;
            if ($inicial_control_atestado_aluno->contr_control_atestado_aluno->NM_ajax_flag)
            {
                $inicial_control_atestado_aluno->contr_control_atestado_aluno->NM_ajax_info['errList']['geral_control_atestado_aluno'][] = $msg;
                return;
            }
            control_atestado_aluno_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function control_atestado_aluno_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['control_atestado_aluno'])  && '' != $_SESSION['scriptcase']['error_icon']['control_atestado_aluno'])  ? $_SESSION['scriptcase']['error_icon']['control_atestado_aluno']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['control_atestado_aluno']) && '' != $_SESSION['scriptcase']['error_close']['control_atestado_aluno']) ? $_SESSION['scriptcase']['error_close']['control_atestado_aluno'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['control_atestado_aluno']) && '' != $_SESSION['scriptcase']['error_span_title']['control_atestado_aluno']) ? $_SESSION['scriptcase']['error_span_title']['control_atestado_aluno'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['control_atestado_aluno']) && '' != $_SESSION['scriptcase']['error_icon_title']['control_atestado_aluno']) ? $_SESSION['scriptcase']['error_icon_title']['control_atestado_aluno'] : '';
?>
