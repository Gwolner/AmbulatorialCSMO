...
$old_error_handler = set_error_handler("control_paciente_scriptcase_error_handler");

function control_paciente_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['control_paciente']['contr_erro']) && $_SESSION['scriptcase']['control_paciente']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_control_paciente->contr_control_paciente->Apl_com_erro = true;
            if ($inicial_control_paciente->contr_control_paciente->NM_ajax_flag)
            {
                $inicial_control_paciente->contr_control_paciente->NM_ajax_info['errList']['geral_control_paciente'][] = $msg;
                return;
            }
            control_paciente_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function control_paciente_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['control_paciente'])  && '' != $_SESSION['scriptcase']['error_icon']['control_paciente'])  ? $_SESSION['scriptcase']['error_icon']['control_paciente']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['control_paciente']) && '' != $_SESSION['scriptcase']['error_close']['control_paciente']) ? $_SESSION['scriptcase']['error_close']['control_paciente'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['control_paciente']) && '' != $_SESSION['scriptcase']['error_span_title']['control_paciente']) ? $_SESSION['scriptcase']['error_span_title']['control_paciente'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['control_paciente']) && '' != $_SESSION['scriptcase']['error_icon_title']['control_paciente']) ? $_SESSION['scriptcase']['error_icon_title']['control_paciente'] : '';
?>
