...
$old_error_handler = set_error_handler("form_atendimento_scriptcase_error_handler");

function form_atendimento_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['form_atendimento']['contr_erro']) && $_SESSION['scriptcase']['form_atendimento']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_atendimento->contr_form_atendimento->Apl_com_erro = true;
            if ($inicial_form_atendimento->contr_form_atendimento->NM_ajax_flag)
            {
                $inicial_form_atendimento->contr_form_atendimento->NM_ajax_info['errList']['geral_form_atendimento'][] = $msg;
                return;
            }
            form_atendimento_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_atendimento_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_atendimento'])  && '' != $_SESSION['scriptcase']['error_icon']['form_atendimento'])  ? $_SESSION['scriptcase']['error_icon']['form_atendimento']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_atendimento']) && '' != $_SESSION['scriptcase']['error_close']['form_atendimento']) ? $_SESSION['scriptcase']['error_close']['form_atendimento'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_atendimento']) && '' != $_SESSION['scriptcase']['error_span_title']['form_atendimento']) ? $_SESSION['scriptcase']['error_span_title']['form_atendimento'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_atendimento']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_atendimento']) ? $_SESSION['scriptcase']['error_icon_title']['form_atendimento'] : '';
?>
