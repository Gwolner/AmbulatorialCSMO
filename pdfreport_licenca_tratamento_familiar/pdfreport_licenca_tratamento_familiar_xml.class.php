...
class pdfreport_licenca_tratamento_familiar_xml
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function grava_arquivo()
   {
      ...
      while (!$rs->EOF)
      {
         $this->xml_registro = "<pdfreport_licenca_tratamento_familiar";
...
$_SESSION['scriptcase']['pdfreport_licenca_tratamento_familiar']['contr_erro'] = 'off'; 
   ...
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->familiar))
         {
             $this->familiar = sc_convert_encoding($this->familiar, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " familiar =\"" . $this->trata_dados($this->familiar) . "\"";
   }
   //----- parentesco
   function NM_export_parentesco()
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->parentesco))
         {
             $this->parentesco = sc_convert_encoding($this->parentesco, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " parentesco =\"" . $this->trata_dados($this->parentesco) . "\"";
   }
   //----- cid
