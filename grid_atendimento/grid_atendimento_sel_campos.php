<?php
   ...
   session_start();
    ...
    $Sel_Cmp = new grid_atendimento_sel_cmp(); 
    $Sel_Cmp->Sel_cmp_init();
   
class grid_atendimento_sel_cmp
{
function Sel_cmp_init()
{
   ...
   $tab_blk_campos[] = "aluno";
   $tab_ger_campos['servidor'] = "on";
   $tab_blk_campos[] = "servidor";
   $tab_ger_campos['servidor_terceirizado'] = "on";
   $tab_blk_campos[] = "servidor_terceirizado";
   $tab_ger_campos['visitante'] = "on";
   $tab_blk_campos[] = "visitante";
   $tab_ger_campos['total'] = "on";
   $tab_blk_campos[] = "total";
   $tab_ger_campos['id_atendimento'] = "on";
   $tab_ger_campos['id_servidor_saude'] = "on";
   $tab_ger_campos['tipo_servico'] = "on";
   $tab_ger_campos['tipo_paciente'] = "on";
   $tab_ger_campos['data_hora'] = "on";
   ...
   if (isset($_SESSION['sc_session'][$this->sc_init]['grid_atendimento']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$this->sc_init]['grid_atendimento']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$this->sc_init]['grid_atendimento']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (isset($_SESSION['sc_session'][$this->sc_init]['grid_atendimento']['usr_cmp_sel']) && !empty($_SESSION['sc_session'][$this->sc_init]['grid_atendimento']['usr_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$this->sc_init]['grid_atendimento']['usr_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
...
function Sel_processa_out()
{
   ...
   $arr_temp = array();
   if (!isset($_SESSION['sc_session'][$this->sc_init]['grid_atendimento']['usr_cmp_sel']))
   {
       $_SESSION['sc_session'][$this->sc_init]['grid_atendimento']['usr_cmp_sel'] = array();
   }
   $arr_order = $_SESSION['sc_session'][$this->sc_init]['grid_atendimento']['field_order'];
   $_SESSION['sc_session'][$this->sc_init]['grid_atendimento']['field_order'] = array();
   $campos_sel = explode("@?@", $this->campos_sel);
   foreach ($campos_sel as $campo_order)
   {
       if (isset($tab_ger_campos[$campo_order]))
       {
           $_SESSION['sc_session'][$this->sc_init]['grid_atendimento']['field_order'][] = $campo_order;
       }
   }
   foreach ($arr_order as $campo_order)
   {
       if (!in_array($campo_order, $_SESSION['sc_session'][$this->sc_init]['grid_atendimento']['field_order']))
       {
           $_SESSION['sc_session'][$this->sc_init]['grid_atendimento']['field_order'][] = $campo_order;
       }
   }
   ...
   $_SESSION['sc_session'][$this->sc_init]['grid_atendimento']['usr_cmp_sel'] = $arr_temp;
?>
...
?>
  function scSubmitSelCampos(sPos) {
   ...
   $.ajax({
    type: "POST",
    url: "grid_atendimento_sel_campos.php",
    data: {
...
<?php
   ...
   foreach ($tab_ger_campos as $NM_cada_field => $NM_cada_opc)
   {
       if ($NM_cada_opc != "none")
       {
           if ($NM_cada_opc != "on")
           {
               $prep_combo[strtolower($NM_cada_field)] = $_SESSION['sc_session'][$this->sc_init]['grid_atendimento']['labels'][$NM_cada_field];
           }
       }
   }
...
<?php
   if (isset($_SESSION['sc_session'][$this->sc_init]['grid_atendimento']['field_order']))
   {
       foreach ($_SESSION['sc_session'][$this->sc_init]['grid_atendimento']['field_order'] as $NM_cada_field)
       {
           if ($tab_ger_campos[$NM_cada_field] == "on")
           {
               ...
               {
?>
     <li class="sc_ui_litem scAppDivSelectFieldsEnabled" id="sc_id_itemsel_<?php echo NM_encode_input($NM_cada_field); ?>"><?php echo $_SESSION['sc_session'][$this->sc_init]['grid_atendimento']['labels'][$NM_cada_field]; ?></li>
<?php
               }
               else
               {
?>
     <li class="sc_ui_litem scAppDivSelectFieldsDisabled" id="sc_id_itemsel_<?php echo NM_encode_input($NM_cada_field); ?>"><?php echo $_SESSION['sc_session'][$this->sc_init]['grid_atendimento']['labels'][$NM_cada_field]; ?></li>
<?php
               }
           }
       }
