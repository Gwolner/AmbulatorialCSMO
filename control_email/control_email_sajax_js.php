  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_control_email" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  // ---------- Validate email
  function do_ajax_control_email_validate_email()
  {
    var nomeCampo_email = "email";
    var var_email = scAjaxGetFieldText(nomeCampo_email);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_control_email_validate_email(var_email, var_script_case_init, do_ajax_control_email_validate_email_cb);
  } // do_ajax_control_email_validate_email

  function do_ajax_control_email_validate_email_cb(sResp)
  {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_control_email_validate_email_cb
  // ---------- Form
  function do_ajax_control_email_submit_form()
  {
    if (scEventControl_active("")) {
      setTimeout(function() { do_ajax_control_email_submit_form(); }, 500);
      return;
    }
    scAjaxHideMessage();
    ...
    scAjaxProcOn();
    x_ajax_control_email_submit_form(var_email, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_control_email_submit_form_cb);
  } // do_ajax_control_email_submit_form

  function do_ajax_control_email_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
  ...
  } // do_ajax_control_email_submit_form_cb

  var scStatusDetail = {};

  function do_ajax_control_email_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    }
  } // do_ajax_control_email_navigate_form

  function do_ajax_control_email_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
{
?>
    do_ajax_control_email_restore_buttons();
<?php
}
?>
    if (hasJsFormOnload)
  ...
  } // do_ajax_control_email_navigate_form_cb
  function sc_hide_control_email_form()
  {
    for (var block_id in ajax_block_id) {
      $("#div_" + ajax_block_id[block_id]).hide();
    }
  } // sc_hide_control_email_form
