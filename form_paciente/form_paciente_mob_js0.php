...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_paciente_mob']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
  <input type="hidden" name="script_case_session" value="<?php echo $this->form_encode_input(session_id()); ?>"> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_paciente_mob"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
function cep_cep(num_cep, campos_cep)
{
   ...
   jsrsExecute("form_paciente_mob_cep.php?cep=" + num_cep + "&onchange=s", recebe_cep, "cep", num_cep, false);
}
